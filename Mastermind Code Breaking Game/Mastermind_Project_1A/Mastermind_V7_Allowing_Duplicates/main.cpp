 ...
 * Purpose: Mastermind V.7
 * Allowing duplicate digits to be used. We'll show hints for both in the next version.
 * For now, we just show hints if NO duplicates are used.
 */
...
void shwHnts(char *,char *,const int);          //Shows hints for the guess we made.
...
int main(int argc, char** argv) {
    ...
    if(strcmp(guess,cdeToBrk)==0){
        ...
        //The hints only work for no duplicates right now, so we only show them if useDup is false.
        if(!useDup)
...
void shwHnts(char *guess, char *cdeToBrk, const int PEGS){
    //To hold how many digits are correct and in the right place.
    int rDrP=0;
    //To hold how many digits are correct and in the wrong place.
    int rDwP=0;
    //To hold how many digits are incorrect and in the wrong place.
    int wDwP=0;
    ...
    for(int i=0;i<PEGS;i++){
        for(int j=0;j<PEGS;j++){
            ...
            //We need to specify that i!=j because we've already check this in the loop for rDrP.
            if(cdeToBrk[i]==guess[j]&&i!=j){
                rDwP++;
            }
        }
    }
