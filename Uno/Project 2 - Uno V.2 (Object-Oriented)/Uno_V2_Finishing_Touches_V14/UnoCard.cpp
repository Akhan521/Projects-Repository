...
void UnoCard::setVal(unsigned char v){
    val=v;
}
void UnoCard::setDescr(string desc){
    descr=desc;
}
...
void UnoCard::setCrd(){
    ...
    //Set a numerical value or other descriptions if needed.
    else{
...
void UnoCard::cpyCrd(UnoCard &card){
    ...
    descr=card.getDescr();
    val=card.getVal();
    //Clear the Contents of the Used Card.
    card.setVal(0);
    card.setDescr("");
    card.setClr("");
}
