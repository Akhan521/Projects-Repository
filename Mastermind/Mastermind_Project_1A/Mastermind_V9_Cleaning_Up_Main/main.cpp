 ...
 * File:   main.cpp
 * Author: Aamir Khan
 * Created on March 23, 2022, 12:45 PM
 * Purpose: Mastermind V.9
 * We clean up main and get the program working for code lengths of 6 and 8.
 */
...
void plyMstrMnd(int cdeLnth, bool useDup){
    ...
    mkeGuess(guess,cdeLnth,nTries,useDup);
...
char **setUpBrd(const int ATMPTS, const int cdeLnth){
    ...
    for(int i=0;i<ATMPTS;i++){
        board[i]=new char[cdeLnth+1];   //4 elements per row (4 column) + Null Term.
    }
    //Set up our board:
    for(int i=0;i<ATMPTS;i++){
...
void mkeGuess(char *guess, const int cdeLnth, int &nTries, bool useDup){
    ...
    }while(strlen(temp)!=cdeLnth||(strlen(temp)==cdeLnth&&!isVld)||(!useDup&&hasDup));
