...
int main(int argc, char** argv) {
    ...
    fstream bin; //To write binary data with
    ...
    //End of Rules.
    cout<<"Eleventh Program of My Uno Project 1."<<endl<<endl;
    cout<<"How many cards should each player start off with (Max=10)? ";
    cin>>cards;
    ...
    //Clear the contents of the bin file first
    bin.open(file,ios::trunc);
    bin.close();
    //Open the bin file again to write the deck of cards to.
    bin.open(file,ios::in|ios::out|ios::binary);
    ...
    //Find a random card that will be read from the bin file.
    int rndCrd=rand()%cards;
    //This card is from when we first got cards. (Before we matched any cards.)
    cout<<"Random Card to Find From When We Began: "<<rndCrd+1<<endl;
    UnoCard *chsnCrd=readBin(bin,rndCrd);
    dsply(*chsnCrd,rndCrd);
...
UnoCard setCrd(){
    ...
    switch(x){
        case 1:
        case 2: strcpy(card.color,"Blue");break;
        case 3:
        case 4: strcpy(card.color,"Green");break;
        case 5:
        case 6: strcpy(card.color,"Red");break;
        case 7:
        case 8: strcpy(card.color,"Yellow");break;
        default:strcpy(card.color,"None");break;
    }
    if(strcmp(card.color,"None")==0){
        strcpy(card.descr,"Wild Card");
        card.value=0;
    }
    else{
        card.value=rand()%11+1; //Values Between [1,11]
        //For when values are between 1-9
        strcpy(card.descr,"Normal");
        if(card.value==10){
...
void dsply(UnoCard card,int i){
    //This will be a Wild Card if value = 0.
    //If value = 10, Draw 1. If value = 11, Draw 2.
    if(card.value==0||card.value==10||card.value==11){
...
void copyCrd(UnoCard &card1,UnoCard &card2){
    ...
    strcpy(card2.color,card1.color);
...
void takeTrn(UnoPlayer &plyr,UnoPlayer &oppt,UnoCard &match){
    ...
    cout<<"Card to Match: ";
        ...
        do{
            ...
            cin>>option;
        }while(option>plyr.numCrds||option<1);
        //We use strcmp to test the descr because it's a char array.
        if(strcmp(plyr.deck[option-1].descr,"Wild Card")==0){
            cout<<endl;
            ...
            char clr[SIZE]; //The new color will be stored here.
        ...
        else if(plyr.deck[option-1].value==10&&strcmp(plyr.deck[option-1].color,match.color)==0){
            cout<<endl;
        ...
        else if(plyr.deck[option-1].value==11&&strcmp(plyr.deck[option-1].color,match.color)==0){
            cout<<endl;
        ...
        else if(strcmp(plyr.deck[option-1].color,match.color)==0||plyr.deck[option-1].value==match.value){
            cout<<endl;
...
void wrtBin(fstream &out,UnoCard card){
    out.write(reinterpret_cast<char *>(&card.value),sizeof(unsigned char));
    out.write((card.color),SIZE);
    out.write((card.descr),SIZE);
}
UnoCard *readBin(fstream &in, int card){
    UnoCard *rndm=new UnoCard;
    long cursor=0L;
    //Find where the card is.
    if(card!=0){
        cursor=card*(sizeof(unsigned char)+SIZE+SIZE);
    }
    in.seekg(cursor,ios::beg);
    in.read(reinterpret_cast<char *>(&rndm->value),sizeof(unsigned char));
    cursor+=sizeof(unsigned char);
    in.seekg(cursor,ios::beg);
    in.read((rndm->color),SIZE);
    cursor+=SIZE;
    in.seekg(cursor,ios::beg);
    in.read((rndm->descr),SIZE);
    return rndm;
}
void wrtTxt(fstream &out,UnoCard card,int i){
    if(card.value==0||card.value==10||card.value==11){
        out<<"Card "<<i+1<<" = ["<<card.color<<" "<<setw(5)
                                 <<card.descr<<"]"<<endl;
    }
    else{
        out<<"Card "<<i+1<<" = ["<<card.color<<" "<<setw(2)
               <<static_cast<int>(card.value)<<"]"<<endl;
    }
}