 ...
 * File:   main.cpp
 * Author: Aamir
 * Created on February 10, 2021, 9:00 PM
 * Purpose: Pointers and Dynamic Memory-> LCR V.7
 */
...
int main(int argc, char** argv) {
    ...
    int *tknCntr = new int[numPlyrs]; //Create a dynamic array for token counters
    int *plcings = new int[numPlyrs]; //Create a dynamic array that will hold the sorted copy
    for(int i=0;i<numPlyrs;i++){
        tknCntr[i]=3; //Initialize all elements with 3 tokens
        plcings[i]=0; //Initialize all elements with 0
    }
    //Display Outputs
    if(numPlyrs==3){ //play if there are 3 players
        ...
        cpyLst(tknCntr,plcings,numPlyrs);
        bublSrt(plcings,numPlyrs);
        cout<<"Ranking after round:\n";
        rnking3(plcings,tknCntr,numPlyrs);
        cout<<endl;
        }while(win==false);//Loop until a win occurs
        int end = time(0);
        cout<<"Number of possible combinations with 3 6-sided dice: "<<pow(6,3)<<endl;
        out<<"Number of possible combinations with 3 6-sided dice: "<<pow(6,3)<<endl;
        cout<<"Time Elapsed in Seconds: "<<end-beg<<endl;
        out<<"Time Elapsed in Seconds: "<<end-beg<<endl;
        dsply(out,tknCntr,numPlyrs,mean,numDice);
        cin.ignore();
        ...
        cpyLst(tknCntr,plcings,numPlyrs);
        selcSrt(plcings,numPlyrs);
        cout<<"Ranking after round:\n";
        rnking5(plcings,tknCntr,numPlyrs);
        cout<<endl;
        }while(win==false);//Loop until a win occurs
        int end = time(0);
        cout<<"Number of possible combinations with 3 6-sided dice: "<<pow(6,3)<<endl;
