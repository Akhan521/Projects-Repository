...
void Board::dsplyKings(string ourPiece){
    ...
    for(it=board.begin();it!=board.end();it++){
        ...
        for(itr=it->begin();itr!=it->end();itr++){
            ...
            if((*itr)->getKingStatus()&&(*itr)->getPiece()==ourPiece)
                ...
                kngPos.push_back((*itr)->getPos());
        }
    }
    //To store the size of our vector.
    ...
    if(size>0){ //If we have at least 1 King piece...
        ...
        //We want to display our positions in descending order.
        //I use a quick-sort to sort our pairs in descending order
        //based on the first values of each pair.
        quickSort(kngPos,0,size-1);
    }
    //We now output our positions:
    for(int i=0;i<size;i++){
        //We output the position of each King piece.
        cout<<"("<<kngPos[i].first<<" "<<kngPos[i].second<<") ";
    }
    //If we had at least 1 King piece, we output a newline.
    if(size>0) cout<<endl;
}

/**To swap 2 pairs of integers. We input the 2 pairs we'd like to swap.
 */
void Board::swap(pair<int,int> &p1,pair<int,int> &p2){
    //Swapping the first values.
    int temp=p1.first;
    p1.first=p2.first;
    p2.first=temp;
    //Swapping the second values.
    temp=p1.second;
    p1.second=p2.second;
    p2.second=temp;
}

/**To partition a vector of pairs around a pivot. This function is used
 * within our quick-sort algorithm to partition our vector. We input
 * the vector we'll be partitioning, the starting index, and the 
 * ending index. We put the pivot in its correct position and return
 * its index.
 */
int Board::partition(vector<pair<int,int>> &v,int start,int end){
    //Choosing our pivot to be the 1st value of the last pair.
    int pvt=v[end].first;
    //Our pointers that will be used to partition our array.
    int l=start,r=end-1;
    //Looping while our pointers are not the same.
    while(l!=r){
        //Finding the first value that is < than our pivot from the left.
        while(v[l].first>=pvt && l!=r) l++;
        //Finding the first value that is > our pivot from the right.
        while(v[r].first<=pvt && l!=r) r--;
        //Swapping the 2 pairs in our vector.
        swap(v[l],v[r]);
    }
    //Placing our pivot in its correct position.
    //If our pivot is not the smallest value in our vector, we perform a swap.
    if(pvt>v[l].first){
        //Swapping it into its correct position.
        swap(v[l],v[end]);
    }
    //If the correct position of our pivot is at the end, we leave it alone.
    else l=end;
    //Returning the index of our pivot.
    return l;
}
